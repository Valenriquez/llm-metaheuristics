...
"""
Creates optuna with custumhys
Works well with normal output

The optuna file gets     ```python
Modifies code once created
Must make a shorter class...
"""
...
class MetaheuristicGenerator:
    ...
    def execute_generated_code(self, code, output_folder, iteration, is_optuna):
        ...
        try:
            if is_optuna:
                optuna_output = subprocess.run(['python', file_name], capture_output=True, text=True, timeout=100)
