...
class MetaheuristicGenerator:
    def __init__(self, dimensions, model="codegemma", max_iterations=7):
        self.model = model
        self.max_iterations = max_iterations
        self.client = chromadb.Client()
        self.python_files_collection = self.client.create_collection(name="algorithm_creation", metadata={"hnsw:space": "cosine"})
    ...
    def generate_prompt(self):
        self.task_prompt = f"""
        You are a computer scientist specializing in natural computing and metaheuristic algorithms. Your task is to design a novel metaheuristic algorithm for the bf.{self.dimensions} optimization problem using only the operators and selectors from the parameters_to_take.txt file.
...
# [Your explanation here, each line starting with '#']
    ...
    def self_refine(self, initial_prompt, data, output_folder, number_iteration):
        ...
        """
        print("refinement_prompt", refinement_prompt)
...
if __name__ == "__main__":
    generator = MetaheuristicGenerator(2)
    generator.run()
    logging.basicConfig(level=logging.DEBUG)
