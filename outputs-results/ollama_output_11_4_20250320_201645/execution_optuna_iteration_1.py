...
def objective(trial):
    heur = [
        ...
        ('central_force_dynamic',
            {
                'gravity': trial.suggest_float('gravity', 0.002, 0.02),
                'alpha': trial.suggest_float('alpha', 0.015, 0.03),
                'beta': trial.suggest_float('beta', 1.6, 2.0),
                'dt': trial.suggest_float('dt', 0.9, 1.1)
            },
            'probabilistic'
        ),
        ('differential_mutation',
            {
                'expression': trial.suggest_categorical('expression', ['rand', 'best', 'current-to-best', 'rand-to-best']),
                'num_rands': trial.suggest_int('num_rands', 1, 3),
                'factor': trial.suggest_float('factor', 0.8, 1.2)
            },
            'greedy'
        ),
        ('gravitational_search',
            {
                'gravity': trial.suggest_float('gravity', 1.5, 2.5),
                'alpha': trial.suggest_float('alpha', 0.025, 0.05)
            },
            'all'
        )
    ]
