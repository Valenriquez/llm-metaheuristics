...
def objective(trial):
    heur = [
        ('spiral_dynamic', {'radius': trial.suggest_float('radius', 0.1, 0.9), 'angle': trial.suggest_float('angle', 5, 25), 'sigma': trial.suggest_float('sigma', 0.01, 0.2)}, 'greedy'),
        ('swarm_dynamic', {'factor': trial.suggest_float('factor', 0.3, 0.9), 'self_conf': trial.suggest_float('self_conf', 1, 3), 'swarm_conf': trial.suggest_float('swarm_conf', 1, 3), 'version': trial.suggest_categorical('version', ['inertial', 'constriction']), 'distribution': trial.suggest_categorical('distribution', ['uniform', 'gaussian', 'levy'])}, 'all'),
