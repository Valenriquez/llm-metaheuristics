...
def objective(trial):
    heur = [
     ...
     trial.suggest_categorical('neighborhood', ['metropolis', 'all'])
),
("central_force_dynamic", 
     {"gravity": trial.suggest_float('gravity', 0.001, 0.01), "alpha": trial.suggest_float('alpha', 0.01, 0.03), "beta": trial.suggest_float('beta', 1.2, 1.5), "dt": trial.suggest_float('dt', 0.7, 0.9)}, 
     trial.suggest_categorical('search_strategy', ['probabilistic', 'greedy'])
),
("differential_mutation", 
     {"expression": trial.suggest_categorical('expression', ['rand', 'best', 'current', 'current-to-best', 'rand-to-best', 'rand-to-best-and-current']), "num_rands": 2, "factor": trial.suggest_float('factor', 0.8, 1.0)}, 
     trial.suggest_categorical('selection_strategy', ['inertial', 'constriction'])
),
("gravitational_search", 
     {"gravity": trial.suggest_float('gravity', 1.3, 1.7), "alpha": trial.suggest_float('alpha', 0.02, 0.04)}, 
     trial.suggest_categorical('optimization_strategy', ['rank', 'cost', 'random', 'tournament_2_100'])
),
                ]
