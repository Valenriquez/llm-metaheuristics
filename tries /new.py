...
class MetaheuristicGenerator:
    def __init__(self, benchmark_function, dimensions, model="myllama3:latest", max_iterations=7):
        ...
        self.file_result = "" # Variable to check if the ouput was succesful
        self.extracted_code = "" # Variable to extract the metaheuristic and create the optuna file 
        self.file_result_error = "" # Variable to check the given error
        
        logging.basicConfig(level=logging.DEBUG)
        self.logger = logging.getLogger(__name__)
        
        self.role_prompt = f"""You are a highly skilled computer scientist in the field of natural computing. Your task is to design metaheuristic algorithms, taking into account the already known information.
        If there as an error, you need to fix it.
        Error: {self.file_result_error}
        """
        ...
        """
        #  JUST REMOVED FROM THE LAST LINE    #  IMPORTANT: DO NOT USE ANY MARKDOWN CODE BLOCKS such as ```python or ```. ALL OUTPUT MUST BE PLAIN TEXT.
    ...
    def self_refine(self, data, output_folder, number_iteration):
        current_output = ollama.generate(
            model = self.model,
            prompt = f"Using this data: {data}, respond to this prompt: {self.role_prompt}"
        )

        # NEED TO CHECK WHAT THE ANSWER IS: 
    ...
    def run(self):
        ...
        try:
            ...
            print("important-meta-data", data)
            ...
            print("important-optuna-data", data_optuna)
            ...
            for i in range(max_iterations):
                ...
                self.self_refine(data, output_folder, i)
