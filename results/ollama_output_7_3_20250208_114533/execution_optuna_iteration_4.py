...
def objective(trial):
    heur = [
        ...
        (
            ...
            {
                'expression': trial.suggest_categorical('expression', ['rand', 'best', 'current', 'current-to-best', 'rand-to-best']),
                'num_rands': trial.suggest_int('num_rands', 1, 5),
                'factor': trial.suggest_float('factor', 0.8, 1.4)
            },
            'greedy'
        ),
        (
...
print(f"Valor objetivo: {trial.value}")
print(f"Parámetros utilizados: {trial.params}")