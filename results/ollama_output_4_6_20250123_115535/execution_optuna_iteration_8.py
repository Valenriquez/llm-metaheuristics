...
def objective(trial):
    heur = [
        ...
        ('central_force_dynamic',
         {'gravity': trial.suggest_float('gravity', 0.05, 0.2), 'alpha': trial.suggest_float('alpha', 0.1, 0.5), 'beta': trial.suggest_float('beta', 1.5, 2.5), 'dt': trial.suggest_float('dt', 0.8, 1.0)},
         'metropolis'),

        ('firefly_dynamic',
         {'distribution': trial.suggest_categorical('distribution', ['uniform', 'gaussian', 'levy']), 'alpha': trial.suggest_float('alpha', 0.5, 0.9), 'beta': trial.suggest_float('beta', 1.2, 1.6), 'gamma': trial.suggest_int('gamma', 30, 70)},
         'greedy'),

        ('spiral_dynamic',
         {'radius': trial.suggest_float('radius', 0.7, 0.9), 'angle': trial.suggest_float('angle', 15.0, 25.0), 'sigma': trial.suggest_float('sigma', 0.02, 0.08)},
         'random'),
    ]
